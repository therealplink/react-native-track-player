...
public abstract class ExoPlayback<T extends Player> implements EventListener, MetadataOutput {
    ...
    public void onTimelineChanged(Timeline timeline, Object manifest, int reason) {
       ...
       if((reason == 0 || reason == 2) && !timeline.isEmpty()) {
    ...
    public void onPositionDiscontinuity(int reason) {
        ...
        if(lastKnownWindow != player.getCurrentWindowIndex()) {
            previous = lastKnownWindow == C.INDEX_UNSET || lastKnownWindow >= queue.size() ? null : queue.get(lastKnownWindow);
        ...
        if(Player.DISCONTINUITY_REASON_SEEK_ADJUSTMENT == reason && lastKnownWindow == player.getCurrentWindowIndex()) {
